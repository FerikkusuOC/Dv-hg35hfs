...
# >>> AS FUNÇÕES QUE EU MANDEI APAGAR SEM QUERER VOLTARAM AQUI <<<
def iniciar_servidor(cenas_visuais, faixas_musicais, duracao_total, arquivo_audio_final, arquivo_saida):
    ...
    # Detecção de ambiente Colab/Linux Headless
    AMBIENTE_COLAB = "COLAB_GPU" in os.environ or "COLAB_RELEASE_TAG" in os.environ

    if AMBIENTE_COLAB:
        print(" -> [COLAB] Gerando link público e seguro para a Interface...")
        try:
            from google.colab import output
            # Cria o túnel e imprime um link clicável na saída da célula
            output.serve_kernel_port_as_window(5000, path='/')
        except ImportError:
            print(" -> [AVISO] Módulo google.colab não encontrado. O túnel pode não funcionar.")
    else:
        # Comportamento padrão para Windows (Abre a aba no navegador local automaticamente)
        threading.Timer(1.5, lambda: webbrowser.open("http://127.0.0.1:5000")).start()

    # No Colab, mudamos o host para '0.0.0.0' para garantir que o proxy interno consiga acessar a porta
    host_bind = '0.0.0.0' if AMBIENTE_COLAB else '127.0.0.1'
    app.run(host=host_bind, port=5000, debug=False, use_reloader=False)
...
def listar_biblioteca():
    # >>> ADICIONADO: .mp3, .wav, .m4a, .aac, .flac <<<
...
def iniciar_render():
    ...
    # Passa as músicas atualizadas para o motor de vídeo
...
def listar_musicas():
    biblioteca = []
    # Usa o diretório onde o Python está rodando como base segura
        ...
        for clima in os.listdir(pasta_base):
            ...
            if os.path.isdir(caminho_clima):
                for arquivo in os.listdir(caminho_clima):
                    # Aceita mais formatos para garantir que nada fique de fora
                    if arquivo.lower().endswith(('.mp3', '.wav', '.ogg', '.m4a')):
                        caminho_completo = os.path.join(caminho_clima, arquivo)
                        duracao = 0
                        try:
                            # O eyed3 só funciona com mp3, tentar ler wav quebra ele
                            if arquivo.lower().endswith('.mp3'):
                                import eyed3
                                # Desativa os logs irritantes do eyed3 no terminal
                        ...
                        biblioteca.append({
                            # O replace('\\', '/') garante compatibilidade máxima do caminho no navegador
