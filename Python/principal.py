...
def main():
    ...
    try:
        if input_ritmo:
            # Substitui vírgula por ponto para evitar erro de conversão
            tempo_alvo = float(input_ritmo.replace(',', '.'))
            if tempo_alvo < 0.5: 
                tempo_alvo = 0.5 # Trava de segurança contra cortes de milissegundos
        else:
            tempo_alvo = 3.0
    except ValueError:
    ...
    if escolha == '1':
        ...
        # Passando a variável de ritmo definida pelo usuário para o construtor da timeline
        ...
        # --- CONTROLE DINÂMICO DE RECURSOS (WORKERS) ---
        ...
        def processar_cena_mineracao(i, cena):
            # O extrator_imagens.py agora baixa E gera o grid na sequência!
