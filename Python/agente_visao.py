...
def gerar_grid_3x3_base64(caminho_imagem):
    try:
        ...
        # Reduzimos o tamanho base de 960x540 para 480x270 (75% menos pixels para a IA engolir!)
        ...
        try: fonte = ImageFont.truetype("arial.ttf", 20) # Fonte menor
        ...
        buffer = BytesIO()
        # Quality 40 (O JPEG ficará feio para humanos, mas os padrões matemáticos continuam lá para a IA)
...
# Função extra para comprimir os Grids de Curadoria também (que já vêm do Terminal 2 grandes)
    ...
    for tentativa in range(2):
        try:
            ...
            if res.status_code == 200:
                ...
                if DEBUG_MODE: print(f"      [DEBUG] [Visão Curadoria] Resposta bruta: '{resposta_ia}'")
                ...
                if "!!!" in resposta_ia or not numeros_encontrados or numeros_encontrados[0] > 5:
                    if DEBUG_MODE: print(f"      [DEBUG] [Aviso] Alucinação detetada. A acionar protocolo de segurança (Tentativa {tentativa+1}/2)")
